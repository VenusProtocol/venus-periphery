...
if (FORK_MAINNET) {
  ...
  forking(config.forkBlock, () => {
    ...
    describe(`EBrake Fork Tests (${config.label})`, () => {
      ...
      // ═════════════════════════════════════════════════════════════════════
      // BNB-ONLY: setCFZero (Diamond with poolId)
      // ═════════════════════════════════════════════════════════════════════
      ...
      describe("setCFZero", () => {
        ...
        it("should revert for unlisted market", async () => {
          ...
          await expect(
            eBrake.connect(whitelistedUser).setCFZero("0x0000000000000000000000000000000000000001", CORE_POOL_ID),
          ).to.be.revertedWithCustomError(eBrake, "MarketNotListed");
        });

        it("should revert for invalid pool ID", async () => {
          const { eBrake, whitelistedUser } = get();
          await expect(eBrake.connect(whitelistedUser).setCFZero(config.vToken1, 999)).to.be.reverted;
        });
        ...
        if (config.emodeConfig) {
          ...
          it(`should revert setCFZero on e-mode pool where market is not listed (poolId=${emode.unlistedPoolId})`, async () => {
            const { eBrake, whitelistedUser } = get();
            await expect(
              eBrake.connect(whitelistedUser).setCFZero(emode.unlistedVToken, emode.unlistedPoolId),
            ).to.be.revertedWithCustomError(eBrake, "MarketNotListed");
          });
        }
      });
